...
pacman::p_load(devtools,tidyverse,knitr,skimr,readxl,dplyr,magrittr,writexl)
thematic::thematic_rmd()
```

```{r}
pacman::p_load(lubridate,ggplot2,sf,treemap,d3r,htmltools,rgeos,maptools,sp,maps,spdep,MASS,spmoran,spatialreg,coda,sphet,ggmap,mapproj,RColorBrewer)
```
```{r}
pacman::p_load(ggsn,rlang,tigris,leaflet,classInt,rgeoda,gridExtra,grid,summarytools,googledrive,tmap,rgdal,mapview,GWmodel,regclass,viridis,GGally)
```
...
covid19<-read_excel('/Users/carolina/My Drive/8/planeación estratégica/Módulo 1/actividad 1/covid19_confirmados.xlsx')
denue_hosp<-read_excel('/Users/carolina/My Drive/8/planeación estratégica/Módulo 1/actividad 1/denue_hospitales (1).xlsx')
```
...
covid_hospital%<>%mutate(Covid_2021=jan_2021+feb_2021+mar_2021+april_2021+may_2021+
  ...
  rename(Pobreza=porcentaje_pob_pobreza,Acceso_SS=porcentaje_pob_acceso_ss,Densidad_2020=popden2020,Poblacion=poblacion_2022)
```
...
```{r message=FALSE, warning=FALSE}
ggpairs(covid_hospital,columns = 5:14,ggplot2::aes(colour=Region),progress=FALSE)
```
...
```{r message=FALSE, warning=FALSE}
accss_global<-ggplot(covid_hospital,aes(Accceso_SS))+geom_histogram(color='black',fill='#07529C')+theme_light()+labs(title='Porcentaje de Población con Acceso a Servicios de Salud',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
pobr_global<-ggplot(covid_hospital,aes(Pobreza))+geom_histogram(color='black',fill='#07529C') +theme_light()+labs(title='Pobreza',subtitle='Nivel Nacional')+xlab('')+ ylab('')
...
```{r message=FALSE, warning=FALSE}
hosp_global<-ggplot(covid_hospital,aes(Hospitales))+geom_histogram(color='black',fill='#07529C')+theme_light()+labs(title='Hospitales',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
cov_global<-ggplot(covid_hospital,aes(Tasa_Covid19_2021))+geom_histogram(color='black',fill='#07529C')+theme_light()+labs(title='Tasa de casos confirmados de Covid-19',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
```{r}
den_global<-ggplot(covid_hospital,aes(Densidad_2020))+geom_histogram(color='black',fill='#07529C')+theme_light()+labs(title='Densidad de Población',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
```{r message=FALSE, warning=FALSE}
cov_acc_gl<-ggplot(covid_hospital,aes(x=Tasa_Covid19_2021,y=Accceso_SS))+geom_point(size=1.5,shape=21,color='black',fill='#07529C')+theme_light()+labs(title='Porcentaje de Población con Acceso a Servicios de Salud',subtitle='Nivel Nacional')+ylab('')+xlab('Covid-19')
...
cov_pobr_gl<-ggplot(covid_hospital,aes(x=Tasa_Covid19_2021,y=Pobreza))+geom_point(size=1.5,shape=21,color='black',fill='#07529C')+theme_light()+labs(title='Población en Pobreza',subtitle='Nivel Nacional')+ylab('')+xlab('Covid-19')
...
```{r message=FALSE, warning=FALSE}
cov_hosp<-ggplot(covid_hospital,aes(x=Tasa_Covid19_2021,y=Hospitales))+geom_point(size=1.5, shape=21,color='black',fill='#07529C')+theme_light()+labs(title='Hospitales',subtitle='Nivel Nacional')+ylab('')+xlab('Covid-19')
...
cov_den<-ggplot(covid_hospital,aes(x=Densidad_2020,y=Hospitales))+geom_point(size=1.5, shape=21,color='black',fill='#07529C')+theme_light()+labs(title='Densidad de Población',subtitle='Nivel Nacional')+ylab('')+xlab('Covid-19')
...
```{r message=FALSE, warning=FALSE}
viol_ass<-ggplot(covid_hospital,aes(y=Accceso_SS))+geom_boxplot(color='black',fill='#07529C',notch=TRUE)+scale_x_continuous(breaks=NULL)+theme_light()+labs(title='Porcentaje de Población con Acceso a Servicios de Salud',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
viol_pobr<-ggplot(covid_hospital,aes(y=Pobreza))+geom_boxplot(color='black',fill='#07529C',notch=TRUE)+scale_x_continuous(breaks=NULL)+theme_light()+labs(title='Población en Pobreza 2022',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
```{r message=FALSE, warning=FALSE}
viol_hosp<-ggplot(covid_hospital,aes(y=Hospitales))+geom_boxplot(color='black',fill='#07529C',notch=TRUE)+scale_x_continuous(breaks=NULL)+theme_light()+labs(title='Hospitales',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
viol_covid<-ggplot(covid_hospital,aes(y=Tasa_Covid19_2021))+geom_boxplot(color='black',fill='#07529C',notch=TRUE)+scale_x_continuous(breaks=NULL)+theme_light()+labs(title='Tasa de casos confirmados de Covid-19',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
```{r}
viol_den<-ggplot(covid_hospital,aes(y=Densidad_2020))+geom_boxplot(color='black',fill='#07529C',notch=TRUE)+scale_x_continuous(breaks=NULL)+theme_light()+labs(title='Densidad de Población',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
```{r message=FALSE, warning=FALSE}
qq_ass<-ggplot(covid_hospital,aes(sample=Accceso_SS))+geom_qq(fill='black',colour='#07529C')+geom_qq_line(linetype='dashed',color='black')+theme_light()+labs(title='Porcentaje de Población con Acceso a Servicios de Salud',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
qq_pobr<-ggplot(covid_hospital,aes(sample=Pobreza))+geom_qq(fill='black',colour='#07529C')+geom_qq_line(linetype='dashed',color='black')+theme_light()+labs(title='Porcentaje de Población en Pobreza',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
```{r message=FALSE, warning=FALSE}
qq_hosp<-ggplot(covid_hospital,aes(sample=Hospitales))+geom_qq(fill='black',colour='#07529C')+geom_qq_line(linetype='dashed',color='black')+theme_light()+labs(title='Hospitales',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
qq_covid<-ggplot(covid_hospital,aes(sample=Tasa_Covid19_2021))+geom_qq(fill='black',colour='#07529C')+geom_qq_line(linetype='dashed',color='black')+theme_light()+labs(title='Tasa de casos confirmados de Covid-19',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
```{r}
qq_den<-ggplot(covid_hospital,aes(sample=Densidad_2020))+geom_qq(fill='black',colour='#07529C')+geom_qq_line(linetype='dashed',color='black')+theme_light()+labs(title='Densidad de Población',subtitle='Nivel Nacional')+xlab('')+ylab('')
...
base<-read.csv('/Users/carolina/Library/CloudStorage/GoogleDrive-a01720509@tec.mx/Shared drives/Planeación estratégica/Módulo 1/ACTIVIDAD 3/datos_act3/covid_hospital.csv')
```
...
## *Modelos de regresión Espaciales*

```{r message=FALSE, warning=FALSE, include=FALSE}
mx_mpios_map<-readOGR(dsn='/Users/carolina/Library/CloudStorage/GoogleDrive-a01720509@tec.mx/Shared drives/Planeación estratégica/Módulo 1/ACTIVIDAD 3/datos_act3/mx_geo_data.shp')
```
...
### *Auto Regresión Espacial Global*
Este modelo (Auto Regresión Espacial) es el que se comparará con el anterior.
...
```{r}
spatial_autoregressive <- lagsarlm(tasa_covid ~ .-CVE_ENT-IDUNICO-municipio-CODELAG-region-grado_rs-entidad, 
                                   data = mx_mpios_map, listw = rswm_queen, Durbin = FALSE) 
...
(gwr_rmse<-Metrics::rmse(gwr_sf$y_predicted,mx_mpios_map$tasa_covid))
...
Usando el RMSE del modelo Random Forest y el modelo Espacial Autoregresivo, podemos ver que el mejor es **Random Forest**. La diferencia es de 0.613669.

```{r}
cat("RMSE del modelo Espacial Autoregresivo Global: ",sar_rmse,"  RMSE del modelo Random Forest:",rf_rmse)
```
...
* Se realizó un modelo de regresión linear, usando todas las variables independientes a excepción de `cve_Ent`, `municipio` y `entidad`. El resultado demostró que `densidad2020` y la cantidad de `hospitales` no son relevantes para explicar el comportamiento de la variable dependiente. 

* El resultado de este modelo evidenció que la `tasa de casos confirmados de Covid-19`, la `población`, la `Región Centro-Norte` y la `Región Sur` no son variables relevantes para explicar el comportamiento de la variable dependiente. Dentro de esta regresión las variables que más peso tienen son `Región Norte` y `Crimen`.

* Usando el RMSE del modelo Random Forest y el modelo Espacial Autoregresivo, podemos ver que el mejor es `Random Forest`. La diferencia es de 0.613669, lo cual indica que es posible que con más variables el otro modelo puede llegar a ser el indicado.

* Analizando el efecto del modelo GWR y las variables independientes en la región centro se observan diferencias que el resto del país.

En la región centro podemos ver que al igual que la mayoría del país, los valores son de entre 0 y 2.
```{r}
mapview(gwr_sf, zcol="densidad_2_TV")
```
En la región centro el valor es entre 0 y -2.
```{r}
mapview(gwr_sf, zcol="poblacion_TV")
```
En la región centro el valor es de entre 0 y -2
```{r}
mapview(gwr_sf, zcol="densidad_2_TV")
```
En la región centro el valor es de entre -1 y 2.
```{r}
mapview(gwr_sf, zcol="inclusion__TV")
```

En la región centro podemos ver como el rezago social afecta entre -2 y 2. Es la variable con más rango. 
```{r}
mapview(gwr_sf, zcol="rezago_soc_TV")
```
